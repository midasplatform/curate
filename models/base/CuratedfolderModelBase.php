...
abstract class Curate_CuratedfolderModelBase extends Curate_AppModel
{
     ...
     */
    public function enableFolderCuration($folderDao)
    {
     ...
     * requires ADMIN access to the folder.
     */
    public function disableFolderCuration($folderDao)
    {
        if (is_null($folderDao)) {
     ...
     */
    public function requestCurationApproval($folderDao)
    {
        if (is_null($folderDao)) {
        ...
        if (count($curatedfolderDaos) == 0) {
            ...
            $body = "Dear qidw.rsna.org admin,\nThe qidw.rsna.org curated folder ".$curatedfolderDao->getName(
                ).' has been requested for approval.  The folder can be found at http://qidw.rsna.org/folder/'.$curatedfolderDao->getFolderId(
                )." .  You will need to log in to view it.\n\nThanks,\nqidw.rsna.org admin\n";
            foreach ($adminDao as $admin) {
                $subject = 'Curated Folder Approval Requested';
                $logger->info(
                    'Sending an email to '.$admin->getEmail().' with subject ['.$subject.'] and body ['.$body.']'
                );
     ...
     * returns the curatedfolderDao.
     */
    public function approveCurationRequest($folderDao, $message = false)
    {
        if (is_null($folderDao)) {
        ...
        if (count($curatedfolderDaos) == 0) {
            ...
            if ($curatedfolderDao->getCurationState() != CURATE_STATE_REQUESTED) {
                throw new Exception(
                    'Curated folder must be in the REQUESTED state before approving curation request.',
                    -1
                );
            }
            ...
            foreach ($policies as $policy) {
                ...
                $body = 'Dear '.$user->getFirstname().' '.$user->getLastname(
                    ...
                    ).' has been approved for curation.  The folder can be found at http://qidw.rsna.org/folder/'.$curatedfolderDao->getFolderId(
                    )." .  You will need to log in to view it.\n\nThanks,\nqidw.rsna.org admin\n";
                $logger->info(
                    'Sending an email to '.$user->getEmail().' with subject ['.$subject.'] and body ['.$body.']'
                );
     ...
     */
    public function listAllCuratedFolders($userDao, $policy)
    {
